...
/*VIENNAMESH INPUT PARAMETERS*/
	...
	if (argc < 3)
	{
		...
		std::cout << "Correct use of parameters: <filename> <cell_size>" << std::endl;
		return -1;
	}
...
*/
	...
	volume_mesh.set_input("cell_size", cell_size);
	volume_mesh.set_input("input_file", filename);
	volume_mesh.run();
	
/*	// Write mesh to file
	viennamesh::algorithm_handle mesh_writer = context.make_algorithm("mesh_writer");
	mesh_writer.set_default_source(volume_mesh);
  	mesh_writer.set_input( "filename", "/home/lgnam/Desktop/software/ViennaMesh/viennamesh-dev/build/examples/data/myfirsttask/partitioned_mesh.vtu" );
  	mesh_writer.run();
*/
