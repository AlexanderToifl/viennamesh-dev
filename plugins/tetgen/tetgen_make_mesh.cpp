...
//     }
    ...
    { 
      #ifdef HAVE_OPENMP
      double tic = omp_get_wtime();	//MY OWN IMPLEMENTATION
      #endif
...
//       tetgen::output_mesh output_mesh;
      ...
      #ifdef HAVE_OPENMP
      double tic_refine = omp_get_wtime();	//MY OWN IMPLEMENTATION
      #endif

      make_mesh_impl( im, om, hole_points, seed_points, options );

      #ifdef HAVE_OPENMP
      double toc_refine = omp_get_wtime();	//MY OWN IMPLEMENTATION
      #endif

      set_output("mesh", output_mesh);
      
      #ifdef HAVE_OPENMP
      double toc_wo_reconversion = omp_get_wtime();	//MY OWN IMPLEMENTATION
      #endif

      // MY OWN IMPLEMENTATION TO OUTPUT NUMBER OF VERTICES AND CELLS
      viennagrid::mesh viennamesh_output;
      viennamesh::convert(output_mesh(), viennamesh_output);
      
      #ifdef HAVE_OPENMP
      double toc_w_reconversion = omp_get_wtime();	//MY OWN IMPLEMENTATION
...
/*
      std::cout << "Number of Vertices: " << viennagrid::vertices(viennamesh_output).size() << std::endl;
      std::cout << "Number of Elements: " << viennagrid::cells(viennamesh_output).size() << std::endl;
      ...
      std::cout << "Time for refinement: " << refine_time << std::endl; */
      ...
      filename="/home/lgnam/Desktop/benchmarks/tetgen_plugin/data/benchmark_tetgen_refine_";
      filename+=tmp;
      filename+=".txt";

      output.open(filename.c_str(), std::ofstream::out | std::ios_base::app);
      output << static_cast<double>(cell_size()) << " " << viennagrid::vertices(viennamesh_output).size() << " " << viennagrid::cells(viennamesh_output).size() << " " << overall_time_w_reconversion <<  \
